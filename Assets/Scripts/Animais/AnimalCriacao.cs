...
public class AnimalCriacao : MonoBehaviour
{
    ...
    [SerializeField] float distanciaProcriacao = 5.0f; // Dist�ncia para considerar outro animal pr�ximo
    ...
    void Update()
    {
        ...
        // Verificar se o animal � um filhote e se o tempo para virar adulto foi atingido
    ...
    private void VerificarParceiroProximo()
    {
        if (parceiroAtual == null || !parceiroAtual.statsGeral.isVivo() || Vector3.Distance(transform.position, parceiroAtual.transform.position) > distanciaProcriacao) //Parceiro nao esta mais perto
        {
            ...
            objIconAmor.SetActive(false);
            return; // Se o parceiro se distanciar, sai do m�todo
        }
        ...
        {
            // Se passar pelas 3 verifica��es, instancia o novo prefab e reinicia o tempo de engorda
